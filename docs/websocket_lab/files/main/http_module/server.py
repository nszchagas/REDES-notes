...
from utils import http_logger as logger, get_reply, format_header_as_str
...
class HTTPHandler(BaseHTTPRequestHandler):
    ...
    def _log_request_info(self, method: str):
        logger.info(f'Method: {method}. Request headers: {format_header_as_str(self.headers)}')


def serve_http(host: str, http_port: int, **kwargs):
    httpd = TCPServer((host, http_port), HTTPHandler)
    logger.info(f'Starting HTTP server at port: {http_port}')
    httpd.serve_forever()
